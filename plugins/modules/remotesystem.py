...
- Performs all remote system operations on a PowerStore Storage System.
  This module supports get details of an existing remote systems, create/Add new
  remote system for all supported parameters, modify remote system with
  supported parameters and delete/remove a specific remote system.
...
options:
  ...
  remote_id:
    ...
    - ID for the remote system is autogenerated, cannot be passed during creation of
      a remote system.
...
- remote_user, remote_port and remote_password are not required during modification,
  getting and deleting the remote system. If passed then these paramters will be
  ignored and the operation will be performed.
- If wait_for_completion is set to True then the connection will be terminated
  after the timeout is exceeded. User can tweak timeout and pass it
  in the playbook task. By default, the timeout is set to 120 seconds.
