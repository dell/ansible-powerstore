...
description:
- Supports the provisioning operations on a filesystem
    Create a filesystem
    Get the details of filesystem
    Modify the attributes of a filesystem
    Delete a filesystem
...
options:        
 ...
 smb_properties:
   ...
   - a)'is_smb_sync_writes_enabled' Indicates whether the synchronous writes 
     option is enabled on the file system.
   - b)'is_smb_no_notify_enabled' Indicates whether notifications of changes 
     to directory file structure are enabled.
   - c)'is_smb_op_locks_enabled' Indicates whether opportunistic file locking 
     is enabled on the file system.
   - d)'is_smb_notify_on_access_enabled' Indicates whether file access 
     notifications are enabled on the file system.
   - e)'is_smb_notify_on_write_enabled' Indicates whether file write 
     notifications are enabled on the file system
   - f)'smb_notify_on_change_dir_depth' Integer variable , determines the 
 ...
 quota_defaults:
   ...
   - a)'grace_period' Grace period of soft limit. 
     '-1' is default , which specifies infinite grace.
     '0' specifies system default of 1 week.
   - b)'grace_period_unit' Unit of the grace period of soft limit.
     Choices are 'days', 'weeks', 'months' . Default is 'days'
   - c)'default_hard_limit' Default hard limit of user quotas and tree quotas.
     Integer type variable , unit is specified through 'cap_unit'.
   - d)'default_soft_limit' Default soft limit of user quotas and tree quotas.
     Integer type variable , unit is specified through 'cap_unit'
   - e)'cap_unit' Capacity unit for default hard & soft limit.
     Options are 'GB' , 'TB'. Defaults to 'GB'
   type: dict 
 state:
   description:
...
filesystem_details:
    ...
    contains:
        ...
        protection_policy:
            description:
                - id and name of the protection policy associated with this filesystem
            type: dict
        nas_server:
            description:
                - id and name of the nas server the filesystem belongs to
            type: dict
        size_total:
            description:
...
class PowerStoreFileSystem(object):
    ...
    def to_modify_filesystem(self, filesystem_details):
        ...
        try:
            ...
            LOG.info("Modify Dict {0}".format(modify_parameters))
    ...
    def perform_module_operation(self):
        ...
        if fs_details:
            ...
            if to_modify_dict:
                to_modify = True
        LOG.info("FileSystem Details: {0} , To Modify {1}".format(
            fs_details, to_modify))
