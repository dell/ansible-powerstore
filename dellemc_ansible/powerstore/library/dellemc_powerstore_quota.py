...
options:
  path:
    ...
    - For user quota, if path is not specified, quota will get created at the
  ...
  filesystem:
    description:
    - The ID/Name of the filesystem for which Tree/User Quota  will be created.
  ...
  description:
    description:
    - Additional information can be mentioned for a Tree Quota
...
- When ID of the filesystem is passed then nas_server is not required.
  If passed then filesystem should exist for the nas_server, else the task will fail.
- If a parent directory of the current directory or a child directory of the
  path is having a Tree Quota configured then quota for that path can't be
  created. Hierarchical tree quotas are not allowed.
- When first quota is created for a directory/User in a filesystem then the
  quotas will be enabled for that filesystem automatically.
- If user quota is to be created on tree quota then user quotas will be
  enabled automatically in tree quota.
...
quota_details:
    ...
    contains:
        ...
        file_system:
            description:
                - Includes ID and Name of filesystem and nas server for which Tree/User exists.
            type: complex
            sample:
            "file_system": {
            "filesystem_type": "Primary",
            "id": "5f1d158f-c6b7-a895-5cbf-42079d64ae37",
            "name": "sample_filesystem",
            "nas_server": {
                "id": "5ef4b912-971a-e2ea-436e-3286ff9e3ccf",
        ...
        tree_quota_for_user_quota:
            description:
                - Additional Information of Tree Quota limits on which User Quota exists.
                - Only applicable for User Quotas
            type: complex
        size_used:
            description:
        ...
        state:
            description:
                ...
                - OK: No quota limits are exceeded.
                - Soft_Exceeded: Soft limit is exceeded, and grace period is not expired.
                - Soft_Exceeded_And_Expired: Soft limit is exceeded, and grace period is expired.
                - Hard_Reached: Hard limit is reached.
            type: str
            sample: "Ok"
        state_l10n:
            description:
