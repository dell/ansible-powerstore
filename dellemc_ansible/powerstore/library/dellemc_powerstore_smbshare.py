...
version_added: '2.8'
short_description:  Manage SMB shares on PowerStore storage system
...
options:
  ...
  umask:
    ...
    - During creation, if not mentioned then default is "022".
    - For all other operations, default is None.
    type: str
  state:
    description:
...
notes:
- When ID of the filesystem/snapshot is passed then nas_server is not required.
  If passed, then filesystem/snapshot should exist for the nas_server,
